...
>(
  (
    ...
    // Handler for sidebar click when clickToToggle is enabled
    const handleSidebarClick = React.useCallback(
      (event: React.MouseEvent) => {
        // Only toggle if clickToToggle is enabled
        if (clickToToggle) {
          // Prevent toggling when clicking on interactive elements or their children
          const isInteractiveElement = (event.target as HTMLElement).closest(
            'button, a, input, select, textarea, [role="button"]'
          );

          // Only toggle if the click was directly on the sidebar background (not on an interactive element)
